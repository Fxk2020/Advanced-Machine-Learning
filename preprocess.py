...
def addId(infileStr):
    ...
    :return:
    """
    csv_file = open(infileStr, 'r', encoding='utf-8')
...
def train_id(infile1, infile2):
    ...
    :return:
    """
    content_id = addId(infile1)
    label_id = addId(infile2)
...
def stopword():
    ...
    """
    stopword = []
    txt_file = open('wordfiles/baidu_stopwords.txt', encoding='utf-8')
...
def getTestDataset(str):
    ...
    for item in csv_read:
        ...
        temp1 = re.sub('[a-zA-Z0-9]', '', item[0])
        temp1 = re.sub('[\s+\.\!\/_,$%^*(+\"\'；：“”．]+|[+——！，。？?[\]《》：()|<>｜【】「」¥＂:、~@#￥%……&*（）]+', '', temp1)

        train_dataset.append([temp1, item[1]])

    to_train = pd.DataFrame(train_dataset)
    to_train.to_csv('testfiles2/taobao1.csv', index=False, header=['text', 'label'])

    train_dataset = []
    stopwordlist = stopword()
    csv_file = open('testfiles2/taobao1.csv', 'r', encoding='utf-8')
    csv_read = csv.reader(csv_file)
    for item in csv_read:
        if csv_read.line_num == 1:
            continue
        ...
        seg_sentence = ''
        seg_list = jieba.cut(item[0], cut_all=False)
        ...
        train_dataset.append([seg_sentence.strip(), item[1]])
    to_train = pd.DataFrame(train_dataset)
    to_train.to_csv('testfiles2/taobao2.csv', index=False, header=['text', 'label'])
...
if __name__ == '__main__':
    ...
    # split_csv('trainfiles/Train_Joint.csv', 'torchtextfiles/train.csv', 'torchtextfiles/val.csv', seed=999, ratio=0.1)

    print(getTestDataset("testfiles2/taobao.csv"))
